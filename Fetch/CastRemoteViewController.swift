...
//        })
    ...
    func updatePlayPauseImage() {
        if castHandler!.isPlaying {
            playBtn.setImage(pauseImage, for: UIControlState())
        } else {
            playBtn.setImage(playImage, for: UIControlState())
        }
    }
...
//        scrubBar.setThumbImage(active, forState: .Highlighted)
    ...
    func addOneSecondToPosition(_ sender: AnyObject?) {
        position += 1
        self.updateScrubBar()
    ...
    @IBAction func seekTo(_ sender: AnyObject) {
        positionTimer?.invalidate()
        seekToPosition(TimeInterval(scrubBar.value))
    }
