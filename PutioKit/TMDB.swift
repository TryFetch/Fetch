...
//        let delay = DispatchTime.now(dispatch_time_t(DISPATCH_TIME_NOW), Int64(Double(TMDB.sharedInstance.requests / 4) * Double(NSEC_PER_SEC)))
        // TODO
...
//        let delay = DispatchTime.now(dispatch_time_t(DISPATCH_TIME_NOW), Int64(Double(TMDB.sharedInstance.requests / 4) * Double(NSEC_PER_SEC)))
    ...
    func searchWithParams(_ params: [String:String], type: String, callback: @escaping ((movie: Movie?, tvshow: TVShow?)) -> Void) {
        ...
        Alamofire.request("\(TMDB.api)search/\(type)", parameters: params)
                        ...
                        if results.count > 0 && type == "movie" {
                            ...
                            if let date = results[0]["release_date"].string {
                                ...
                                if let parsed = formatter.date(from: date) {
                                    ...
                                    movie.releaseDate = year.description
                                }
                            }
...
//        let delay = DispatchTime.now() + UInt64(Double(TMDB.sharedInstance.requests / 4) * Double(NSEC_PER_SEC))
        // TODO

        DispatchQueue.main.asyncAfter(deadline: .now()) {
            
            Alamofire.request("\(TMDB.api)tv/\(showId)/season/\(season)/episode/\(episode)", parameters: params)
                .responseJSON { response in
